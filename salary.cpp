...
void add()    //增加职工工资数据
{
	...
	/*if(i=1)
	{
		printf("增加第：1");    //逐项给第n条结构体数组中的属性赋值
		printf("工号：");
		scanf("%s",zgs[0].num);
		printf("姓名：");
		scanf("%s",zgs[0].name);
		printf("岗位工资：");
		scanf("%f",&(zgs[0].wage_gw));
		printf("薪级工资：");
		scanf("%f",&(zgs[0].wage_xj));
		printf("职务津贴：");
		scanf("%f",&(zgs[0].wage_zw));
		printf("绩效工资：");
		scanf("%f",&(zgs[0].wage_jx));
		zgs[0].wage_yf=zgs[0].wage_gw+zgs[0].wage_xj+zgs[0].wage_zw+zgs[0].wage_jx;
		grsds(0);    //调用个人所得税函数计算个人所得税
		zgs[0].wage_sf=zgs[0].wage_yf-zgs[0].taxfee;
		printf("已增加，注意保存！");
	}
	else if(i>1)
	{*/
